...
depthOfStiffChanges=(300.0,600.0,900.0,1200.0)
depthOfWebChanges=(1200.0,1500.0,1800.0,2100.0)
thicknessChanges=(5,10,15,20)

import part

for thicknessOfStiff in thicknessChanges:
    for depthOfWeb in depthOfWebChanges:
        for depthOfStiff in depthOfStiffChanges:
            ...
            import job
            jobName = 'Code-'+str(int(thicknessOfDeck))+'-'+str(int(thicknessOfStiff))+'-'+str(int(depthOfWeb))+'-'+str(int(depthOfStiff))+'S4R'
            ...
            dispData = dispField.values[upperRightNodes.label-1].data[2]
            dispDataOfUpperRight.append((thicknessOfStiff,depthOfWeb,depthOfStiff,dispData))

            downRightNodes=myInstance.nodes.getClosest((widthX/2-900.0,-heightY/2,0.0),)
            dispData = dispField.values[downRightNodes.label-1].data[2]
            dispDataOfDownRight.append((thicknessOfStiff,depthOfWeb,depthOfStiff,dispData))

            upperLeftNodes=myInstance.nodes.getClosest((-widthX/2+900.0,heightY/2,0.0),)
            dispData = dispField.values[upperLeftNodes.label-1].data[2]
            dispDataOfUpperLeft.append((thicknessOfStiff,depthOfWeb,depthOfStiff,dispData))

            downLeftNodes=myInstance.nodes.getClosest((-widthX/2+900.0,-heightY/2,0.0),)
            dispData = dispField.values[downLeftNodes.label-1].data[2]
            dispDataOfDownLeft.append((thicknessOfStiff,depthOfWeb,depthOfStiff,dispData))
